...
def rank(title,source_type):
 score=55;t=normalized(title)  # Retrieval priority, never a confidence score.
...
def unique_sources(group,sources):
 ...
 for item in group:
  ...
  seen.add(org);rows.append(dict(originalOrganization=item.get('originalOrganization') or org,name=item['sourceName'],url=item['url'],publishedAt=item['publishedAt'],type=item['sourceType'],organization=org,role=source.get('role','evidence' if item['sourceType']=='primary' else 'reporting')))
 return rows
