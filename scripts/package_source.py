...
ROOT=Path(__file__).resolve().parents[1];target=ROOT/'public/downloads/FrameNexo-source.zip';target.parent.mkdir(parents=True,exist_ok=True)
...
with zipfile.ZipFile(target,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=9) as z:
 for rel in sorted(set(files)):
  ...
  path=ROOT/rel
  if path.is_file():z.write(path,'framenexo/'+rel)
