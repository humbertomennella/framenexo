...
import collections,datetime as dt,hashlib,html,json,re,textwrap,time,urllib.error
...
def editorial_cover_media(candidate, persist=True):
 """Generate a unique, code-native editorial cover with traceable rights, never a fake photo."""
 title=str(candidate.get('title') or 'Matéria').strip()
 digest=hashlib.sha256((str(candidate.get('id',''))+'|'+title).encode()).hexdigest()
 lines=textwrap.wrap(title,width=33)[:6]
 texts=''.join(f'<text x="80" y="{220+i*66}" font-size="52" font-weight="700">{html.escape(line)}</text>' for i,line in enumerate(lines))
 svg=f'<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="675" viewBox="0 0 1200 675"><rect width="1200" height="675" fill="#090a0d"/><rect x="80" y="70" width="72" height="5" fill="#b4f564"/><g fill="#e6e8e2" font-family="sans-serif"><text x="80" y="130" font-size="30">APURANTE / {html.escape(str(candidate.get("category","Informação")))}</text>{texts}<text x="80" y="630" font-size="22">Apurante Editorial · Capa tipográfica, não é fotografia</text></g></svg>'
 path=f'/images/news/capa-{digest[:24]}.svg';asset=p.ROOT/'public'/path.lstrip('/');
 if persist:asset.parent.mkdir(parents=True,exist_ok=True);asset.write_text(svg)
 credit='Apurante Editorial · Capa tipográfica original, não é fotografia.'
 rights=p.read('data/image-rights.json',[])
 if persist and not any(row.get('path')==path for row in rights):
  rights.append(dict(path=path,origin='Composição vetorial original do APURANTE',credit=credit,license='Produção própria; autorizada para publicação no APURANTE',sourceURL='https://github.com/humbertomennella/framenexo',scope='Capa tipográfica exclusiva desta matéria; sem fotografia ou elementos externos.',proof=dict(generator='scripts/publish_edition.py:editorial_cover_media',articleTitle=title,candidateId=candidate.get('id'),createdAt=p.iso()),sha256=hashlib.sha256(svg.encode()).hexdigest()))
  p.write('data/image-rights.json',rights)
 return dict(path=path,alt=f'Capa tipográfica: {title}',credit=credit)
 ...
 for group in groups:
  ...
  try:
   ...
   if dry_run:
    ...
    if media is None:media=editorial_cover_media(c,persist=False)
   ...
   p.write(f'data/evidence/{c["id"]}.json',dict(sourceUrls=record['sourceUrls'],sourceOrganizations=record['sourceOrganizations'],sourceSha256=hashlib.sha256(body.encode()).hexdigest(),sharedFact=p.read(f'.cache/drafts/{review_id}-shared-fact.json',{}),verifiedAt=stamp,claims=[f['claim'] for f in draft['facts']],review=review,model='Qwen3-4B-Q4_K_M',checks=['numeric anchors','copy detection','format validation',route_check,'same-model factual review','event deduplication']))
   count+=1
