...
def verify(group,sources,cache=None):
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        jobs={executor.submit(p.evidence,c,sources[c['sourceId']]):c for c in pending}
        for future in as_completed(jobs):
            item=jobs[future]
            try:cache[item['url']]=future.result()[:3500]
    ...
    for item in group:
        ...
        # A detected wire attribution cannot become another independent newsroom.
