...
with tempfile.TemporaryDirectory(prefix='framenexo-github-') as temp:
 ...
 run(['git','init','-b','main'],target);run(['git','config','user.name',login],target);run(['git','config','user.email',f'{login}@users.noreply.github.com'],target);run(['git','add','.'],target);run(['git','commit','-m','feat: launch Eixo Fato editorial portal'],target)
