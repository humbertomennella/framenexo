...
export interface Article {
 ...
 sources:{name:string;url:string;publishedAt:string;type:string;organization?:string;role?:string}[]; verificationPolicyVersion?:number; leadSourceOrganization?:string; corrections?:{date:string;text:string}[]; body:string; Content:any;
}
...
export const independentSourceCount = (a:Article) => new Set(a.sources.map(s=>s.organization||(()=>{try{return new URL(s.url).hostname}catch{return s.name}})())).size;
